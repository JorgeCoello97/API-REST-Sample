...
function getProduct (req, res) {
  ...
  Product.findById(productId, (err, product) => {
    if (err) return res.status(500).send({message: `Error performing the request: ${err}`})
    if (!product) return res.status(404).send({message: `The product requested, doesn't exist.`})
...
function getProducts (req, res) {
  Product.find({}, (err, products) => {
    if (err) return res.status(500).send({message: `Error performing the request: ${err}`})
    if (!products) return res.status(404).send({message: "There aren't products."})
